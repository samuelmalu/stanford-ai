...
def distinct_words(corpus):
    ...
    # ### START CODE HERE ###
    corpus_words_set = { word for corpus_segment in corpus for word in corpus_segment }
...
def compute_co_occurrence_matrix(corpus, window_size=4):
    ...
    # ### START CODE HERE ###
    word2Ind = {word : index for word, index in zip(words, range(num_words))}
    M = np.zeros((num_words, num_words))

    #each corpus segment
        #traverse words
         #current word not in words? ignore
         #get left window
         #get right window
         #merge windows
         #loop words in window 
            #increment M[current-word][window-word]

    for corpus_segment in corpus:
        for index, current_word in enumerate(corpus_segment):
            if current_word not in word2Ind:
                continue
            left_index = max(0, index - window_size)
            left_window = corpus_segment[left_index:index]
            right_index = min(len(corpus_segment), index + window_size + 1)
            right_window = corpus_segment[index + 1 : right_index]
            window = left_window + right_window
            
            for window_word in window:
                M[word2Ind[current_word]][word2Ind[window_word]]+=1
...
def reduce_to_k_dim(M, k=2):
    ...
    svd = TruncatedSVD(k, n_iter=n_iter)
