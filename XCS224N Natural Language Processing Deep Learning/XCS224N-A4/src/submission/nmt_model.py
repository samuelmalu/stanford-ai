...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        decoder_input_size = embed_size + hidden_size
        self.encoder = nn.LSTM(embed_size, hidden_size, bias=True, bidirectional=True)
        self.decoder = nn.LSTMCell(decoder_input_size, hidden_size, bias=True)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Yt in Yts:
            ...
            (new_hidden, new_cell), combined_output, _ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            dec_state = (new_hidden, new_cell)
            combined_outputs.append(combined_output)
            o_prev = combined_output

        combined_outputs = torch.stack(combined_outputs, dim=0)
        combined_output.to(self.device)
        ### END CODE HERE
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden_rearranged = torch.unsqueeze(dec_hidden, dim=1) #(b, 1, h)
        enc_hiddens_proj_rearranged = torch.permute(enc_hiddens_proj, (0, 2, 1)) #(b, h, src_len)
        e_t = torch.bmm(dec_hidden_rearranged, enc_hiddens_proj_rearranged) #(b, 1, src_len)
