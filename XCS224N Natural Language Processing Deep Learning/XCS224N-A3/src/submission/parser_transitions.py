...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        batch_transitions = model.predict(batch)
        for i in range(len(batch_transitions)):
            batch[i].parse_step(batch_transitions[i])
        unfinished_parses = [x for x in unfinished_parses if not (len(x.stack) == 1 and len(x.buffer) == 0)]
