...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        B, T, C = x.size()
        l1_o = self.w1(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        relu_o = nn.ReLU()(l1_o)

        # print(f"========================")
        # print(f"d: {C}")
        # print(f"l: {T}")
        # print(f"d/h: {C//self.n_head}")
        # print(f"x: {x.shape}")
        # print(f"1_o: {l1_o.shape}")
        # print(f"relu_o: {relu_o.shape}")

        l2_o = relu_o.matmul(self.w2.narrow(1, 0, T)) + self.b2.narrow(0, 0, T)
        att = l2_o.masked_fill(self.mask[:,:,:T,:T] == 0, -1e10) # todo: just use float('-inf') instead?
        att = F.softmax(att, dim=-1)
        v = self.value(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        y = att @ v # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
        y = y.transpose(1, 2).contiguous().view(B, T, C) # re-assemble all head outputs side by side
