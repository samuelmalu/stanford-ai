...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        document_length = random.randint(4, int(self.block_size*7/8))
        document = document[:document_length]

        masked_content_length_ratio = random.normalvariate(0.25, 0.1)
        min_masked_content_length_ratio = 1/document_length
        max_masked_content_length_ratio = (document_length - 2)/document_length

        min_masked_content_length = document_length * min_masked_content_length_ratio
        max_masked_content_length = document_length * max_masked_content_length_ratio
        masked_content_length  = masked_content_length_ratio * document_length

        if masked_content_length < min_masked_content_length:
            masked_content_length = min_masked_content_length
        elif masked_content_length > max_masked_content_length:
            masked_content_length = max_masked_content_length
        
        masked_content_length = int(masked_content_length)

        prefix_length = int((document_length - masked_content_length) / 2)
        suffix_length = document_length - prefix_length - masked_content_length

        prefix_start_index = 0
        masked_content_start_index = prefix_start_index + prefix_length
        suffix_start_index = masked_content_start_index + masked_content_length

        prefix = document[prefix_start_index:  masked_content_start_index]
        masked_content = document[masked_content_start_index:  suffix_start_index ]
        suffix =document[suffix_start_index:]

        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.MASK_CHAR
        masked_string_len = len(masked_string)
        if masked_string_len < self.block_size:
            pad_length = self.block_size - masked_string_len
            masked_string = masked_string + self.PAD_CHAR * pad_length

        input = masked_string[:-1]
        output = masked_string[1:]
        
        input_tensor = torch.tensor([self.stoi[i] for i in input], dtype=torch.long)
        output_tensor = torch.tensor([self.stoi[o] for o in output], dtype=torch.long)
