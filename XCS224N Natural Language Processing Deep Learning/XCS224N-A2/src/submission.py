...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  dot_prods_center_and_outside_vectors = np.dot(outside_vectors, center_word_vec)
  prob_distribution = softmax(dot_prods_center_and_outside_vectors)
  p_o_c = prob_distribution[outside_word_idx]

  loss = -1 * np.log(p_o_c)

  true_distribution = np.zeros_like(prob_distribution)
  true_distribution[outside_word_idx] = 1
  entropy = prob_distribution - true_distribution

  grad_center_vec = np.dot(np.transpose(outside_vectors), entropy)

  grad_outside_vecs = np.zeros_like(outside_vectors)
  for i in range(len(grad_outside_vecs)):
    grad_outside_vecs[i] = center_word_vec * entropy[i]
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  for word in outside_words:
     ...
     local_loss, local_grad_center_vecs, local_grad_outside_vectors = word2vec_loss_and_gradient(center_word_vec, outside_word_idx, outside_vectors, dataset)
     loss += local_loss
     grad_center_vecs[center_word_idx] += local_grad_center_vecs
     grad_outside_vectors += local_grad_outside_vectors
